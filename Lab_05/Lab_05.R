...
which((Vect$x == NA | Vect$x == 0), arr.ind = TRUE)
#Q2c
...
for(t in Totally_200){
  if(Emissions$Total[t] <= 300){
    Totally_200[t] -> Total_E[C]
  } 
  C + 1 -> C
}
...
for(o in seq(2,1000)){
  n[o-1] + (r * n[o-1]) - (a * n[o-1] * p[o-1]) -> n[o]
  p[o-1] + (k * a * n[o-1] * p[o-1]) - (m * p[o-1]) -> p[o]
}
